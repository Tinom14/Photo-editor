...
def test_register_login_user(user_data):
    register_url = f"{BASE_URL}/register"
    login_url = f"{BASE_URL}/login"
    user_json = user_data
    response = requests.post(register_url, json=user_json)
    assert response.status_code == 201
    response = requests.post(login_url, json=user_json)
    assert response.status_code == 200
    data = response.json()
    assert 'token' in data
...
@pytest.fixture(scope='function')
def created_task_id(auth_token):
    task_url = f"{BASE_URL}/task"
    headers = {'Authorization': f'Bearer {auth_token}'}
...
def test_task_status_and_result(auth_token, created_task_id):
    status_url = f"{BASE_URL}/status/{created_task_id}"
    result_url = f"{BASE_URL}/result/{created_task_id}"
    headers = {'Authorization': f'Bearer {auth_token}'}
